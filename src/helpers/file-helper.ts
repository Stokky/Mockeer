...
const getFixtureFile = (filePath: string, fileName: string) => path.join(filePath, `${filenamify(fileName, { replacement: '' })}.json`);

//const getFixtureFolder = (fixturesPath, fixturesFolder) => path.join(fixturesPath, fixturesFolder);
const getFixtureFolder = (fixturesPath: string, fixturesFolder: string) => path.join(fixturesPath, fixturesFolder);
